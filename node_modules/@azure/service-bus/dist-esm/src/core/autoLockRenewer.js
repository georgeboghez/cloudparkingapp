...
export class LockRenewer {
    ...
    start(linkEntity, bMessage, onError) {
        try {
            ...
            logger.verbose(`${logPrefix} Total autolockrenew duration for message with id '${bMessage.messageId}' is: ${new Date(totalAutoLockRenewDuration).toString()}`);
            const autoRenewLockTask = () => {
                ...
                if (renewalNeededToMaintainLock && haventExceededMaxLockRenewalTime) {
                    if (linkMessageMap.has(bMessage.messageId)) {
                        ...
                        // when the execution of user's message handler completes.
                        const autoRenewTimer = setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                            try {
                    ...
                    logger.verbose(`${logPrefix} Current time ${new Date()} exceeds the total autolockrenew duration ${new Date(totalAutoLockRenewDuration)} for message with messageId '${bMessage.messageId}'. Hence we will stop the autoLockRenewTask.`);
